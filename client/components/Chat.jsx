...
function Chat({ gameManager }) {
  ...
  useEffect(() => {
    ...
    console.log('[Chat] Setting up chat message handler, gameManager:', gameManager);
    if (gameManager) {
  ...
  const handleSubmit = (e) => {
    ...
    console.log('[Chat] gameManager.sendChatMessage:', gameManager?.sendChatMessage);
    ...
    if (trimmedMessage && gameManager && gameManager.sendChatMessage) {
      ...
      console.error('[Chat] Cannot send message - missing gameManager or sendChatMessage');
    }
  };
  ...
  return (
    <div className={`chat-container ${isMobile ? 'mobile' : 'desktop'} ${isInputFocused && isMobile ? 'input-focused' : ''}`}>
