...
export class ArenaShootingGame {
  ...
  onUpdate(deltaTime) {
    ...
    if (!this.lastLogTime || Date.now() - this.lastLogTime > 1000) {
      console.log("[ArenaShootingGame] onUpdate called, deltaTime:", deltaTime);
    ...
    if (!this.gameStarted) {
      ...
      console.log("[ArenaShootingGame] Players found:", players.length);
      if (players.length > 0) {
        this.isHost = true; // For now, everyone acts as host for their view
        this.gameStarted = true;
        console.log("[ArenaShootingGame] Game started! isHost:", this.isHost);
      }
      return;
    }
    ...
    if (this.isHost) {
      ...
      if (!isNaN(deltaTime) && deltaTime > 0) {
        ...
        if (!this.lastTimerLog || Date.now() - this.lastTimerLog > 1000) {
          console.log("[ArenaShootingGame] Spawn timer progress:", this.spawnTimer.toFixed(2), "/", this.spawnInterval);
          this.lastTimerLog = Date.now();
        }
        
        if (this.spawnTimer >= this.spawnInterval) {
          console.log("[ArenaShootingGame] Time to spawn enemy! Timer:", this.spawnTimer);
          this.spawnEnemy();
          this.spawnTimer = 0;
        }
      } else {
  ...
  spawnEnemy() {
    ...
    console.log("[ArenaShootingGame] Spawning enemy:", enemyId, "at position:", position);
  ...
  createEnemy(id, position) {
    console.log("[ArenaShootingGame] Creating enemy at position:", position, "height:", position.y);
    ...
    console.log("[ArenaShootingGame] Enemy created. Total enemies:", this.enemies.size);
  }

  updateEnemies(deltaTime) {
    const players = this.api.getEntitiesWithComponents(ComponentTypes.POSITION, ComponentTypes.PLAYER);
    ...
    this.enemies.forEach((enemy, id) => {
      if (players.length > 0) {
        ...
        players.forEach(playerId => {
          const playerPos = this.api.getComponent(playerId, ComponentTypes.POSITION);
  ...
  handleShoot(event) {
    ...
    const players = this.api.getEntitiesWithComponents(ComponentTypes.POSITION, ComponentTypes.PLAYER, ComponentTypes.VRM);
    ...
    console.log("[ArenaShootingGame] Local players found:", localPlayers.length);
  ...
  createProjectile(id, position, direction) {
    console.log("[ArenaShootingGame] Creating projectile at height:", position.y, "direction:", direction);
  ...
  checkCollisions() {
    ...
    this.projectiles.forEach((projectile, projId) => {
      this.enemies.forEach((enemy, enemyId) => {
        const distance = projectile.body.position.distanceTo(enemy.body.position);
        if (distance < 0.4) { // Hit! (0.3 enemy radius + 0.1 projectile radius)
